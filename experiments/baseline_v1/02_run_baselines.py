"""
02_run_baselines.py — Run BM25, Dense, and Hybrid retrieval on prepared benchmark data.

Logs per-query results for all three retrievers so we can compute the oracle.

This is the CORE experiment script. Every number in your future paper
starts from the results this script produces.

Usage:
    cd D:\LLM-RESEARCH-LAB\efficient-rag-pipeline
    python experiments/baseline_v1/02_run_baselines.py
    
    # Or run on just one dataset to test:
    python experiments/baseline_v1/02_run_baselines.py --dataset nq
    
    # Change top_k:
    python experiments/baseline_v1/02_run_baselines.py --top_k 5
"""
...
# CONFIG
# ============================================================
DATA_DIR = "experiments/baseline_v1/data"
RESULTS_DIR = "experiments/baseline_v1/results"
CACHE_DIR = "experiments/baseline_v1/cache"
TOP_K_VALUES = [1, 3, 5, 10]   # Evaluate at multiple k values
EMBEDDING_MODEL = "all-MiniLM-L6-v2"

# ============================================================
# METRICS — These are the standard metrics used in retrieval papers
# ============================================================
def normalize_answer(text):
    """Lowercase, strip punctuation and articles for fair comparison."""
    import re
...
def exact_match_score(prediction, ground_truths):
    ...
    for gt in ground_truths:
        ...
        if normalize_answer(gt) in pred_norm or pred_norm in normalize_answer(gt):
            return 1.0
    return 0.0
...
def check_hit(retrieved_contents, answers):
    """Check if any answer appears in any retrieved passage."""
...
def compute_mrr(retrieved_contents, answers):
    """Mean Reciprocal Rank — at what rank is the first relevant result?"""
...
def build_chunk_list(corpus_texts):
    """Convert list of passage strings to chunk dicts matching your pipeline format."""
    chunks = []
...
def run_single_query_all_retrievers(question, corpus_texts, encoder, top_k=5):
    """
    ...
    # --- Hybrid (RRF) ---
    t0 = time.time()
    # Need more candidates for fusion
    dense_candidates = faiss_ret.retrieve(query_embedding, top_k=max(10, top_k * 2))
    sparse_candidates = bm25.retrieve(question, top_k=max(10, top_k * 2))
...
def run_baseline_experiment(dataset_name, data, encoder, top_k_values):
    ...
    """
    print(f"\n{'='*60}")
    print(f"RUNNING BASELINE: {dataset_name.upper()}")
    print(f"{'='*60}")
    ...
    for i, item in enumerate(tqdm(data, desc=f"Evaluating {dataset_name}")):
        ...
        # Run all retrievers at max k (we'll slice later for different k values)
        max_k = max(top_k_values)
        retriever_results = run_single_query_all_retrievers(
            question, corpus, encoder, top_k=max_k
        )
        
        # Compute metrics at each k for each retriever
        query_result = {
            "query_id": i,
            "question": question,
            "answers": answers,
        ...
        for ret_name in ["bm25", "dense", "hybrid"]:
            ...
            for k in top_k_values:
                ...
                em = exact_match_score(concat_text, answers)
        ...
        # Progress print every 100 queries
        if (i + 1) % 100 == 0:
...
def main():
    ...
    parser.add_argument("--top_k", type=int, nargs="+", default=TOP_K_VALUES,
                        help="Top-K values to evaluate at")
    ...
    # Load encoder once (reuse across all queries)
    ...
    print(f"  Next step: python experiments/baseline_v1/03_analyze_oracle.py")
